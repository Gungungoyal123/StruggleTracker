...
function Dashboard() {
  ...
  const [tab, setTab] = useState("struggling"); // "struggling" or "mastered"
...
const fetchUser = async () => {
  try {
    ...
    const response = await fetch("http://localhost:8000/user/profile", {  // your profile endpoint
      method:"GET",
      headers: { Authorization: `Bearer ${token}` },
      
    });
    const data = await response.json();
    if (response.ok) setUsername(data.name); // adjust based on your response shape
  } catch (error) {
    console.log(error);
  }
};
  const fetchProblems = async () => {
    try {
      const token = localStorage.getItem("mytoken");
      const response = await fetch("http://localhost:8000/user/getproblem/struggling", {
        headers: { Authorization: `Bearer ${token}` },
      });
        const response2 = await fetch("http://localhost:8000/user/getproblem/mastered", {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await response.json();
      const data2=await response2.json();
      if (response.ok) {
        setstrugglingProblems(data.data);
      }
      if (response2.ok) {
        setmasteredproblems(data2.data);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  // Filter problems by current tab
  const filteredProblems = tab==="struggling"?strugglingproblems:mastered;

  // Today's due problems (you can update this logic based on your backend)
